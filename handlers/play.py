...
def updated_stats(chat, queue, vol=100):
    if chat.id in callsmusic.pytgcalls.active_calls:
        stats = "⌯  اعداداتــك لـ ڪِـروب : **{}**".format(chat.title)
        if len(que) > 0:
            stats += "\n\n"
            stats += "⌯  الحــجم : {}%\n".format(vol)
            stats += "⌯  الأغانـي في قائمـة الانتظار : `{}`\n".format(len(que))
            stats += "⌯  يـتم تشغيل الان اغنية : **{}**\n".format(queue[0][0])
            stats += "⌯  تم طلب تشغيل الاغنيه من قبل : {}".format(queue[0][1].mention)
    else:
        stats = None
    return stats
...
def r_ply(type_):
    ...
    mar = InlineKeyboardMarkup(
        [
            ...
            [
                InlineKeyboardButton("⌯  معلومات الاغنية  .", "playlist"),
            ],
            [       
...
async def m_cb(b, cb):
    ...
    if type_ == "pause":
        ...
        if chet_id in callsmusic.pytgcalls.active_calls:
            ...
            callsmusic.pytgcalls.leave_group_call(chet_id)
            await cb.message.edit("⏹ **تم الـتعطيل!**")
        else:
            await cb.answer("الحســاب المساعــد  لم يتصــل بعــد!", show_alert=True)
...
async def play(_, message: Message):
    ...
    if audio:
        ...
        keyboard = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton("⌯ عـࢪض القـائمة  .", callback_data="menu"),
                    InlineKeyboardButton("⌯ اخـفاء القـائمة  .", callback_data="cls"),
                ],[
                    InlineKeyboardButton("⌯ قـناه السورس  .", url=f"https://t.me/nnnnBn")
                ],
            ]
        )
        file_name = get_file_name(audio)
        title = file_name
        thumb_name = "https://telegra.ph/file/b71a382a49ce656cc61a1.jpg"
        ...
        try:
            ...
            keyboard = InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton("⌯  انقر هنا لتشغيل الاغنية الاولى : 🎙", callback_data=f'plll 0|{query}|{user_id}'),
                    ],
                    [
                        InlineKeyboardButton("⌯  انقر هنا لتشغيل الاغنيه الثانيه : 🎙", callback_data=f'plll 2|{query}|{user_id}'),
                    ],
                    [
                        InlineKeyboardButton("⌯  انقر هنا لتشغيل الاغنيه الثالثه : 🎙", callback_data=f'plll 3|{query}|{user_id}'),
                    ],
    ...
    if chat_id in callsmusic.pytgcalls.active_calls:
        ...
        await message.reply_photo(
            photo="final.png",
            caption=f"⌯ **اسم الاغنية : ** [{title[:35]}]({url})\n⏱ **Duration:** `{duration}`\n♥️**Status:** `Playing`\n" \
...
async def lol_cb(b, cb):
    ...
    if cb.from_user.id != useer_id:
        await cb.answer("⌯  أنت لست من طلب هذه الأغنية للتحكم بها !", show_alert=True)
        return
    ...
    if chat_id in callsmusic.pytgcalls.active_calls:
        ...
        photo="final.png",
        caption=f"⌯ **اسم الاغنية : ** [{title[:35]}]({url})\n⏱ **Duration :** `{duration}`\n♥️**Status :** `Playing`\n" \
               +f"⌯ **الشخص الذي طلب الاغنيه هو : ** {r_by.mention}",
        reply_markup=keyboard,
        )
        os.remove("final.png")
